...
function JobTitle() {
  ...
  const [employementTypes, setEmploymentTypes] = useState<EmploymentTypeProps>({
    "Full Time": "",
    "Part Time": "",
    Contract: false,
    Internship: false,
  ...
  useEffect(() => {
    const selectedTypes = Object.keys(employementTypes).filter((type) => {
        return employementTypes[type as keyof EmploymentTypeProps];
    })
        ...
        <div className="flex-1 flex flex-col gap-2">
          {Object.entries(employementTypes).map(([type, checked]) => (
            ...
            >
              ...
              />
              <Label htmlFor="employmentType" className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">
                {type }
              </Label>
            </div>
          ))}
